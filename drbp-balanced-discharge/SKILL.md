...
Use the `run_script` tool to execute `scripts/battery_monitor.py`, which outputs JSON statistics of the battery status. **Example**: ```python run_script("scripts/battery_monitor.py") ```
...
```json
{
  ...
  "reason": "采用均衡策略，选择每个模组SOC最高的4个电芯，组成4串2并拓扑。电压匹配度95%，电流裕量20%，最弱电芯SOC裕量8%。"
}
```
