---
name: drbp-agent
description: 使用 agent 规划动态可重构电池组的连接方式。根据导航信息和电池状态选择策略（高功率、高能量、均衡、热管理等），执行电芯挑选算法，生成标准化输出。适用于电动汽车、储能系统的电池管理系统（BMS）优化场景。
---
...
- **v_req**: 满足当前导航任务的需求电压（V），基于车辆模型计算
- **i_req**: 满足当前导航任务的需求电流（A），基于功率需求和电压计算
- **selected_cells**: 选中的电芯，按模组组织，每个模组内的 `cells` 为二维列表，每个子列表为一个串联起来的支路
